...
только с `st`. `get_messages_updates` возвращает события как есть, с
`event_count`, `active` и `more_available`; `wait_seconds` до 30.
Завершайте наблюдение `unsubscribe_messages` — как и любую другую подписку.
