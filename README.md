...
Он предоставляет 50 инструментов:
...
- `get_previous_messages` — страница сообщений платформы старше указанной даты;
- `subscribe_messages` — подписка на новые сообщения платформы и смену их статуса;
- `get_messages_updates` — чтение накопленных событий сообщений;
- `unsubscribe_messages` — завершение подписки на сообщения;
...
- `include_read` — по умолчанию `false`: Viking отдаёт только непрочитанные;
  `true` добавляет прочитанные (`read=true` в запросе). Для исторических и
  ретроспективных вопросов («последнее уведомление о рестарте», «что сообщала
  платформа вчера», «покажи историю уведомлений») передавайте
  `include_read=true` — иначе уже прочитанные записи потеряются; `false`
  оставляйте для вопроса «есть ли новые/непрочитанные уведомления»;
...
Страница сообщений платформы старше указанной даты — `messages.get_previous`
(api.md, раздел 11.13.3). Дополняет `get_messages_history`, когда дата начала
неизвестна: «покажи последние уведомления» — это `older_than = сейчас`.
Для следующей страницы передайте самое раннее `dt` полученной.

Параметры:

- `older_than` в ISO 8601 с обязательным часовым поясом — преобразуется в
  `mt` формата `epoch_msec`; нижней границы нет;
- `include_read`, `limit` (1..100) и `timezone` — как у `get_messages_history`,
  включая правило: ретроспективные вопросы — `include_read=true`.

Ответ в том же формате: `state`, `dt_iso`, `count_in_database`, все поля
сообщения сохраняются.

### `subscribe_messages`, `get_messages_updates`, `unsubscribe_messages`

Подписка `messages.subscribe` (api.md, раздел 11.13.1) — способ дождаться
нового уведомления платформы, не опрашивая историю. Подписка уровня учётной
записи: параметров нет. Снапшот содержит до 20 непрочитанных сообщений,
`max_time` (`mt` — максимальное время в базе, может быть `null`) и `count`.

В обновлениях Viking присылает ключ сообщения `msg` и только изменившиеся
поля: новое сообщение приходит с `st`/`dt`, смена статуса прочитано/непрочитано —
только с `st`. `get_messages_updates` возвращает события с `event_count`,
`active` и `more_available`; `wait_seconds` до 30.

Схема строк та же, что у истории: в `messages` каждого события `st` дублируется
читаемым `state`, `dt` — `dt_iso` (параметр `timezone`, по умолчанию
`Europe/Moscow`), `max_time` дополняется `max_time_iso`. В обновлении `state`
и `dt_iso` есть только у тех сообщений, для которых пришли `st`/`dt`.
`values` и `data` остаются как прислал Viking. Завершайте наблюдение
`unsubscribe_messages` — как и любую другую подписку.
