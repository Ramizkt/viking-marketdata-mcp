...
Он предоставляет 56 инструментов:

- `get_authorization_status` — текущие права подключения без запроса к Viking;
...
или выключать существующие подключения вне текущего MCP. Некоторые
...
Обычный **Stop portfolios** без указания направления сохраняет прежнее поведение:
...
Семантика остановок: [интерфейс платформы](https://fkviking.github.io/bot-doc/docs/getting-started.html).
