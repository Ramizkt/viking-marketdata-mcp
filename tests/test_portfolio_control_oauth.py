...
def config(tmp_path, enabled=True):
    return Settings(
        public_base_url="http://127.0.0.1:8000", export_signing_key="fixture-only-encryption-key",
        oauth_client_store_path=tmp_path / "oauth-clients.json", viking_portfolio_writes_enabled=enabled,
    )


def oauth_client(scopes):
    return OAuthClientInformationFull(
        client_id="fixture-client", client_secret=None,
...
def params(scopes):
    return AuthorizationParams(
        state="fixture-state", scopes=scopes, code_challenge="fixture-challenge",
        redirect_uri=AnyUrl("http://127.0.0.1:9876/callback"),
        redirect_uri_provided_explicitly=True, resource="http://127.0.0.1:8000/mcp",
    )
...
async def test_browser_consent_and_grant_round_trip(tmp_path, monkeypatch, mode, write):
    ...
    app = Starlette(routes=[Route(
        "/oauth/connect/{pending_id:str}", provider.connect_page, methods=["GET", "POST"]
    )])
    ...
    with TestClient(app) as browser:
        ...
        response = browser.post(f"/oauth/connect/{pending_id}",
                                data={**form, "allow_portfolio_writes": "yes"}, follow_redirects=False)
...
@pytest.mark.parametrize("registered, requested, enabled", [
...
async def test_read_refresh_cannot_gain_write_scope(tmp_path):
    ...
    issued = provider._issue_session_tokens(
        ...
        client_id="fixture-client", scopes=[OAUTH_SCOPE], resource=None,
        subject="fixture-subject", absolute_expires_at=4_000_000_000,
    )
