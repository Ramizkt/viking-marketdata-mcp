...
@pytest.mark.parametrize(
    "action, kind",
    [
        ("hard_stop", "portfolio.hard_stop"),
        ("stop_formulas", "portfolio.formulas_stop"),
    ],
)
async def test_special_commands_use_exact_api_types(client, action, kind):
    ...
    assert client._request_connected.await_args.args == (
        kind,
        {"r_id": TARGET["robot_id"], "p_id": TARGET["portfolio"]},
    )
...
async def test_preview_does_not_send(client):
    result = await PortfolioControlService(client).run(
        targets=[TARGET], action="user_fields", fields={"uf0": 1}
    )
    assert result["status"] == "preview"
    assert result["items"][0]["request"]["data"]["portfolio"] == {
...
async def test_batch_reports_rejection_and_unknown_and_remaining(client):
    ...
    client.execute_portfolio_control = AsyncMock(
        side_effect=[
            acknowledgement(),
            error,
            WriteOutcomeUnknown(TimeoutError("fixture")),
        ]
    )
...
@pytest.mark.parametrize(
    "enabled, scopes, confirmation, expected",
    [
...
async def test_mcp_requires_flag_scope_and_confirmation(
    client, monkeypatch, enabled, scopes, confirmation, expected
):
...
async def test_mcp_field_patch_survives_schema_validation(client, monkeypatch):
    ...
    async with create_connected_server_and_client_session(main.mcp, raise_exceptions=True) as session:
        result = await session.call_tool(
            ...
            {
                **TARGET,
                "fields": {"uf0": 0, "uf1": {"c": ""}, "uf19": {"v": -1.5, "c": "test"}},
            },
        )
...
async def test_user_field_execution_sends_only_patch(client):
    ...
    assert client._request_connected.await_args.args == (
        "portfolio.update",
        {"r_id": TARGET["robot_id"], "portfolio": {"name": TARGET["portfolio"], "uf0": {"c": "x"}}},
    )
    client.get_portfolio_template.assert_awaited_once_with(**TARGET)
